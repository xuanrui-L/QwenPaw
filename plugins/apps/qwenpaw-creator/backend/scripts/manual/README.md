...
- `run_real_short_drama_e2e.py` runs the three-second or one-minute Creator
  short-drama flow with qwen-image3 and HappyHorse. Add `--prompt-only` to the
  `minute60` profile to call only the real text model, keep media dispatch
  disabled, and audit variable-duration main-Agent prompts before generation.
