...
def test_parse_object_grounding_maps_normalized_boxes_to_pixels():
    ...
    assert detections == [
        {
            ...
            "bbox_pixel": [10, 40, 90, 160],
        },
    ]


def test_parse_object_grounding_supports_aliases_and_rejects_bad_boxes():
    detections = object_grounding.parse_object_grounding(
        """```json
        {"objects":[
    ...
    assert detections == [
        {
            "label": "cat",
            "bbox_normalized": [0, 0, 500, 1000],
            "bbox_pixel": [0, 0, 40, 40],
        },
    ]
...
def test_parse_object_grounding_supports_ref_box_fallback():
    ...
    assert detections[0]["bbox_normalized"] == [10, 20, 500, 600]
...
def test_ground_image_objects_uses_creator_vlm_and_returns_raw_response(
    ...
    result = asyncio.run(
        object_grounding.ground_image_objects(
            _png_bytes(),
            "/generated/projects/project-1/task-work/request-1/input.png",
            "all people",
        ),
    )
