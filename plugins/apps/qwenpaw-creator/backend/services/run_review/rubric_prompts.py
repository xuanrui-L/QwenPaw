...
5. 这是建议不是门禁：输出只用于驱动下一轮修订，不阻断交付。低全帧差异、静态机位或一张姿态静止的图片不能证明视频冻结，人物说话时的嘴型微动可能低于全帧阈值。music_detected 的频谱/节拍投票只是启发式，带音高的人声也会命中，未收到可听音频不能断言添加了背景音乐。句首到切点的距离不是口型同步测量；不得据此断言声画错位。客观提示须结合真实证据，无法确认时不驱动重做。{probe_discipline}
