...
_GRID = re.compile(r"(?<!\d)([1-9])\s*(?:列\s*)?[x×*]\s*([1-9])(?!\d)", re.I)
_SINGLE = re.compile(r"单格分镜|单(?:张|幅)(?:静态)?关键帧|单帧分镜")
...
def declared_storyboard_panel_count(prompt: str) -> int | None:
    ...
    counts = {_number(match.group(1)) for match in _CELLS.finditer(prompt)} - {
        0,
    }
    counts.update(
        int(match.group(1)) ** 2
        for match in _GRID.finditer(prompt)
