...
| `mm_plugins/renderers/data.py` | `.../renderers/data.py` | Emits PIL table images + markdown text blocks; sheet ordinal exposed as page number; table rendering prefers an installed CJK font; empty cells render blank instead of "nan"; the markdown row cap is overridable via `max_rows` for full-text indexing. |
...
| `mm_plugins/renderers/code.py` | `.../renderers/code.py` | Verbatim aside from block meta additions; the line cap is overridable via `max_lines` for full-text indexing. |
