...
角色/镜头填空。格数由动作中间过程与衔接点决定，不由文本段落数量决定。对白为主或
单一互动的镜头，优先用 2×2 四格覆盖起始、关键动作、反应和末态；四格足够时不要扩成
九格，也不要按每秒或每句台词分配一格。只有存在四格容纳不了、确实不可省略的多个
状态转折时再选 3×3；不为增加格数改变实际镜头结构或补演新动作。减少无信息差异的
重复面孔与手部互动，为身份和道具保留足够清楚的画面空间。禁止
masonry、英雄大格、身份板式非对称布局或任何混合尺寸排布。

除非剧情明确要求双胞胎、分身或同一角色的多个实体，同一格内每个已命名角色只能
出现一个视觉实例；跨格重复表示时间推进，不得在某一格克隆角色或用多重曝光表现
运动轨迹。多角色同框时，用可见差异固定每个人的发型、脸型、胡须和服装对应关系；
不能让某格把甲的脸换到乙的衣服上。返修出现串脸、复制纸币或手部粘连的分镜时，
优先缩减到必要关键状态、简化手部互动，复用已批准身份参考，不盲目增加格数和描述。

每个面板明确当前动作相位和相对前一格的变化。以下项目如已由全局合同固定，不必
逐格重写；只补充该格实际不同或影响理解的内容：
...
背景、人物、服装、声音或镜头。
职责只能描述该张图实际包含的物件，不能因为故事需要就声称它提供另一个资产的外观。
另有独立身份图的关键道具应真实绑定该道具和所选变体，再更新实际参考列表与编号；
例如杯盘参考不能代替单独建立的笔记本参考。
...
视频提示词逐字保留**已确认的本集正文**中的对白，注明说话者、语言、语气、口型与
说话前后表演，不在这个阶段擅自改词或把台词写进分镜图。改编原始长剧本时，若用户
允许精编，应先在正文阶段保留因果、人物关系与关键台词，删减重复叙述并提交审阅；
逐字保留不意味着把原文全部对白塞进短片。另列环境音、关键动作音、音乐进入/退出点与有意静默。
声音应服务动作与情绪，不让每一秒都塞满对白或音效。

逐段把所有说话者的台词加起来核算，不能给每个角色分别占用一整段的时长。先留动作、
接话与表情停顿，再按中文自然对白约 3–4 字/秒估算；方言和情绪表演更慢。演唱要单独
安排延音、换气和伴奏，不用普通说话速度估算，也不把普通 TTS 试听当成已完成演唱。
超出容量时回到正文精编或调整时长，不能寄希望于模型加速、截句或同时说话。
核算用实际说话区间的结束减开始，不能拿整镜时长作分母，也不能只写一个希望达到的
语速数字。例如 20 个汉字安排在第 5–9 秒，就是 20÷4=5 字/秒；应留至少 5–7 秒的
说话时间或先提交精编，不能把它标成 3.5 字/秒。改动动作时序后，重新核算剩余对白
时间，并同步 narrative、storyboard_prompt 和 video_prompt 的动作、说话起止与末态。
短镜头有两人接话时，把第一句安排在开头约 0.3–0.8 秒内进入，允许添茶等简单动作
与说话自然并行；不要先安排数秒的无声动作，再把两句对白挤在结尾。为最后一句留出
完整说完及约 0.5 秒收尾的余量。若实际成片首句明显延迟或第二句缺失，先改首帧和
动作/对白时序，必要时用说话者近景对切或拆镜；不能仅追加“不得截断”后原样重试。
有角色音频参考时只复用其音色，当前台词仍来自本段剧本；保留视频原生对白的区间不
叠加第二份相同台词。首段先实际检查人声、口型与角色对应关系，再扩展到其他剧集。
