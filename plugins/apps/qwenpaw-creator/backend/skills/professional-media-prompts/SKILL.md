...
9:16。外层交付图仍可使用项目画幅；网格自然比例不匹配时用外侧留白与统一 gutter
吸收，绝不拉伸、压扁、裁边、倾斜、合并或把单格改成另一宽高比。先建立一个目标
比例的单格模板，再复制布局。设动作节点数为 `P`，取 `N=ceil(sqrt(P))`，强制使用
严格的 `N×N` 等尺寸槽位，按行优先放入 `P` 个分镜，其余槽位保持纯留白、不得画
占位镜头。5 格必须使用 3×3 槽位并留下 4 个纯留白槽位；禁止 2+3/3+2、masonry、
英雄大格、身份板式非对称布局或任何混合尺寸排布。
