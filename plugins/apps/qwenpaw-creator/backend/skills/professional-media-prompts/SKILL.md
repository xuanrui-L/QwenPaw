...
9:16。外层交付图仍可使用项目画幅；紧凑网格的自然比例不匹配时用外侧留白与统一
gutter 吸收，绝不拉伸、压扁、裁边、倾斜、合并或把单格改成另一宽高比。先建立一个
目标比例的单格模板，再复制为全部等尺寸面板。多格图选择接近方形且不浪费带框槽位的
紧凑行列数：横屏 6 格通常为 3 列×2 行、12 格为 4 列×3 行；竖屏可转为 2 列×3 行。
末行不足时居中已有面板，空余面积属于无边框的外层留白，不得画占位镜头或复制角色/
镜头填空。禁止 masonry、英雄大格、身份板式非对称布局或任何混合尺寸排布。这里选择
紧凑规则网格是为了降低模型布局歧义；保持单格画幅依靠等尺寸模板与外侧留白，并不依赖
方阵。
...
排除项和真实提交顺序。先处理 storyboard，再处理角色、场景、道具、动作/运镜参考。
参考素材中的文字标签不能替代职责合同。**不存在通用 `image1` 语法**：只有模型
...
- 是否使用 `N×N` 等尺寸槽位并保持多余槽位纯留白，同一格也没有意外克隆角色？
