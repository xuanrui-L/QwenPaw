...
镜头、动作阶段和分镜关键帧不是同一个数量。一个 6 秒连续镜头可以保持 1 Shot，
在分镜图中用多个可读关键帧展示同一动作的推进。不要为了凑格数新增 Shot、延长
...
角色/镜头填空。格数由动作中间过程与衔接点决定，不由 shots 数决定，不为了 4 或 9 格
...
上面是默认链；显式版本列表优先且保持顺序。视频仍在该列表前放本镜头已选
分镜图，再按 exact version 稳定去重；分镜图生成本身不放入已选分镜图。

引用参考图统一写 `[Image 1]`、`[Image 2]` …，编号就是该图在参考图序列中的位置
（仅视频输入的 storyboard 恒为 `[Image 1]`）。**不要写任何模型的原生语法**：Runtime 在提交前
...
同一规则也适用于 `storyboard_prompt` 和视觉资产图：写 `[Image N]`，不要写死
“第一张参考图”“第二张参考图”或“图1”等供应商表达。分镜图编号来自该阶段
`storyboard_reference_version_ids` 的实际有序输入，同一 exact version 稳定去重；
未指定列表时才采用实际解析的阵容/角色/场景/道具锚点。待生成的分镜图不是自己
的输入，禁止套用视频的分镜图首位规则或聚合 inputRefs 的顺序。Runtime 图片适配
会把 Qwen 引用改为图N、Gemini/FLUX 改为 image N，只有有序图片输入而没有特殊
标记的模型使用自然语言序数；作者只维护同一 canonical 形式。缺失图片仍占原编号，
必须先修复引用；超出当前模型数量上限须明确缩减并重写对应编号，不能静默丢图。
...
- 单个连续 Shot 是否用足够关键帧覆盖动作中间过程、转折、结束与下一镜头衔接，且没有为格数改变 Shot 和时长？
