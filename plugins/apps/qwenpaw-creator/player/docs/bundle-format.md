...
编辑器每页只展示一个页面生成提示词，界面、动效及需要增加的功能统一在这里描述，没有额外的整体提示词或增加功能选择器。旧项目未填写逐页提示词时，以共享 `design_prompt` 为编辑起点，保存后写入该页 `screens[page].design_prompt`。生成时共享设计仅为风格背景，逐页要求优先。选中页面功能按钮后只显示一个“按钮生成提示词”编辑区，包含“按钮文案：”与“外观与动效：”；一次保存同时更新 control 的 `label` 与 `design_prompt`，保留生成时的明确文案校验，不把旧 label 留作冲突约束。
