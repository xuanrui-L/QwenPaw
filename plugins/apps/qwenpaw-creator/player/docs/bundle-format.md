...
编辑器每页只展示一个页面生成提示词，界面、动效及需要增加的功能统一在这里描述，没有额外的整体提示词或增加功能选择器。旧项目未填写逐页提示词时，以共享 `design_prompt` 为编辑起点，保存后写入该页 `screens[page].design_prompt`。生成时共享设计仅为风格背景，逐页要求优先；选中已有按钮后，详情切换到该按钮的文案和外观提示词。
