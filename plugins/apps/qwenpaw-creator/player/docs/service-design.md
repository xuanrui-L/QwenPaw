...
> 部署实施归平台。本节只记录 **ivb 自身的状态形态与约束**——即“ivb 能不能正确跑起来”
> 依赖哪些存储事实，供平台安排时参考，不是部署步骤。对平台的完整诉求见
...
本节只写 **ivb 自身的契约与事实**。平台侧如何满足、网关与部署如何安排不由本文规定，
见 `platform-integration.md`。

- **ivb 不鉴权**（§1 决策 1）：只消费请求里带来的用户标识，当前从 HTTP 头 `X-User-Id` 读取。
  缺头时放映类端点落 `anonymous`（开发期无网关也能玩），上传端点返回 401。
- **依赖的一条性质**：到达 ivb 的用户标识应由可信上游依据已认证结果决定，终端用户无法自行
  指定。这条性质 ivb 无法自证——头是网关写的还是浏览器写的，在字节上完全相同。
- **失效方式是静默的**：性质不成立时 ivb 不报错、测试全绿，只是任何人都能读写他人进度、
  冒名上传。所以它属于**对接约定**，而不是运行时校验。
- **放映必须对用户可达**：观众在浏览器里拉包、拉 Range 分段、上报进度，所以“ivb 完全不
  对用户开放”并不成立，身份来源仍需在上游某处确定。这是约束，不是实施要求。
- **有状态单实例**（§7.1）：进度库与包体都在本地盘上，多副本或网络文件系统会损坏数据。
