...
const BADGE_META: { type: ModelType; icon: string; label: string }[] = [
...
/**
 * Model readiness indicator from the draft header: a fill-tertiary pill holding
 * one 8px status dot plus one 20px model glyph per model type.  The glyphs are
 * drafted single-colour artwork, so they are painted through a CSS mask and
 * tinted by state.  Clicking the pill still opens the model configuration.
 *
 * The trailing margin keeps the pill clear of the PawApp host capsule pinned to
 * the top-right corner (24px inset, 36px tall).
 */
            ...
            >
              ...
              >
                <span
                  ...
                  style={{ background: ready ? READY_COLOR : IDLE_COLOR }}
                />
              </span>
              <span
                className="h-5 w-5 shrink-0"
                style={{
                  backgroundColor: ready ? READY_COLOR : IDLE_COLOR,
                  // Quoted so inlined `data:` glyphs keep working: an unquoted
                  // url() would break on the `#` of their fill colours.
