...
describe("AgentDock public output and interactions", () => {
  ...
  it.each([true, false])(
    "keeps consecutive native calls together with a loaded user turn: %s",
    (hasUserTurn) => {
      const calls = [
        { name: "read_project", arguments: {} },
        { name: "view_skill", arguments: { skill: "visual-asset-design" } },
        {
          name: "view_skill",
      ...
      seedSession("IDLE", {
        messages: [
          ...(hasUserTurn
            ? [msg({ messageSeq: 1, text: "制作小狗跳跃的画面" })]
            : []),
          ...calls.flatMap((call, index) => [
            asst({
              messageId: `call-message-${index}`,
              ...
              metadata: {
                actionId: `call-${index}`,
                toolCall: call,
                providerThinking: "SECRET_THINKING",
              },
            }),
            msg({
              messageId: `call-result-${index}`,
              messageSeq: index * 2 + 3,
              role: "tool",
              source: "file_agent_runtime",
              text: '{"ok":true,"content":"PRIVATE_GUIDE_CONTENT"}',
              metadata: { toolCallId: `call-${index}`, toolName: call.name },
            }),
          ]),
          asst({
            messageId: "public-narration",
            messageSeq: 8,
            text: "接下来设计小狗的外观。",
          }),
        ],
      ...
      renderDock();
      const rows = [...document.querySelectorAll("[data-agent-tool]")];
      ...
      expect(rows[0].parentElement).toHaveAttribute("data-agent-response-flow");
      expect(rows[0].nextElementSibling).toBe(rows[1]);
      expect(rows[1].nextElementSibling).toBe(rows[2]);
      expect(rows[2].nextElementSibling).toHaveTextContent(
        "接下来设计小狗的外观。",
      );
      expect(
        document.querySelector(".agent-assistant-message:empty"),
      ).toBeNull();
      expect(document.body).not.toHaveTextContent(
        /SECRET_THINKING|PRIVATE_GUIDE_CONTENT|visual-asset-design|professional-media-prompts/u,
      );
    },
  );

  it("shows a real main production authorization as waiting without a fake specialist card", () => {
    seedSession("RUNNING", {
      messages: [msg({ messageSeq: 1, text: "制作三张设计图" })],
      events: [
        evt("agent.tool_started", 1, {
          toolCallId: "production",
          tool: "request_workgraph_execution",
        }),
        evt("execution.authorization_required", 2, {
          toolCallId: "production",
          authorizationId: "image-one",
        }),
      ],
    });
    renderDock();
    const card = document.querySelector('[data-agent-tool="production"]')!;
    expect(card).toHaveTextContent("准备制作");
    expect(card).toHaveTextContent("等待您的确认");
    expect(card.querySelector(".agent-activity-indicator")).toHaveAttribute(
      "data-phase",
      "attention",
    );
    expect(card).not.toHaveClass("agent-specialist-card");
    expect(document.querySelector("[data-agent-live-status]")).toHaveAttribute(
      "data-state",
      "waiting",
    );
  });

  it("shows the current review after an older approval-waiting run was superseded", () => {
    seedSession("PENDING_REVIEW", {
      messages: [msg({ messageSeq: 1, text: "继续制作" })],
      events: [
        evt("agent.tool_started", 1, {
          toolCallId: "old-production",
          runId: "old-run",
          tool: "request_workgraph_execution",
        }),
        evt("execution.authorization_required", 2, {
        ...
        evt("agent.run.cancelled", 3, { runId: "old-run", superseded: true }),
        evt("execution.authorization_decided", 4, {
          toolCallId: "replacement-production",
          authorizationId: "scene",
          status: "APPROVED",
        }),
      ],
    ...
    renderDock();
    const card = document.querySelector('[data-agent-tool="old-production"]')!;
    expect(card).toHaveAttribute("data-status", "cancelled");
    expect(card).toHaveTextContent("已转入后续处理");
    expect(card).not.toHaveTextContent("等待您的确认");
    expect(
      document.querySelector("[data-agent-live-status]"),
    ).toHaveTextContent("审阅");
    expect(
      document.querySelector("[data-agent-live-status]"),
    ).not.toHaveTextContent("确认开始制作");
  });
