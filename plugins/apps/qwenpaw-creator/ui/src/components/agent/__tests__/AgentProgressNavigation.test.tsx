...
describe("overview operation clicks reach the actual generation field", () => {
  ...
  it("opens the exact ungenerated design variant and focuses its prompt from an overview click", async () => {
    const project = structuredClone(
      useProjectSnapshotStore.getState().project!,
    );
    const cat = project.visual.entities.items.cat;
    const variantId = "variant:second";
    cat.variants.items[variantId] = {
      ...cat.variants.items[cat.variants.order[0]],
    ...
    cat.variants.order.push(variantId);
    useProjectSnapshotStore.setState({ project });
    const visual = {
      ...operation("visual", `visual:cat:${variantId}`),
      timelineId: null,
      locator: { page: "assets", assetId: "cat" },
    };
    useWorkGraphStore.setState({
      graph: {
        ...useWorkGraphStore.getState().graph!,
        nodes: [visual],
        counts: { done: 1 },
      },
    });
    const { container } = renderOverview();
    fireEvent.click(
      within(container.querySelector("[data-node-id]")!).getByRole("button"),
    );
    const field = `/visual/entities/items/cat/variants/items/${variantId}/prompt`;
    await waitFor(() => {
      const target = findCreatorFieldElement(
        field,
        container.querySelector("main")!,
      );
      expect(target).toHaveTextContent("第二造型的公开设计提示词");
      expect(target).toHaveClass("review-flash");
    });
    expect(useCreatorInteractionStore.getState().selectedRef).toBe(
      `visual-variant:cat@${variantId}`,
    );
    const query = useNavigationStore.getState().reviewFocus!.query;
  ...
  it("leaves an already-open Plan workbench overlay before focusing the standalone operation prompt", async () => {
    const { container } = renderOverview(
      `/project/p1/t/${encodeURIComponent(
        timelineId,
      )}/plan?element=${elementId}`,
    );
    fireEvent.click(
      within(container.querySelector("main")!).getByRole("button", {
        name: "去制作台编辑",
      }),
    );
    await waitFor(() =>
      expect(container.querySelector("[data-workbench-modal]")).not.toBeNull(),
    );
    expect(container.querySelector("[data-plan-page]")).not.toBeNull();
    clickOperation(container, "storyboard");
    await waitFor(() => {
      expect(container.querySelector("[data-workbench-modal]")).toBeNull();
      expect(container.querySelector("[data-plan-page]")).toBeNull();
      expect(container.querySelectorAll("[data-r2v-workbench]")).toHaveLength(
        1,
      );
      const prompt = findCreatorFieldElement(
        `${base}/storyboard_prompt`,
        container.querySelector("main")!,
      );
      expect(prompt).toHaveClass("review-flash");
      expect(prompt?.closest("[data-workbench-pane=prompt]")).not.toBeNull();
    });
  });
});
