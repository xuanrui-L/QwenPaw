...
收件箱用于集中处理审批事件，并接收定时任务、心跳和新邮件自动处理的执行结果，适合做
统一回看与处理。
...
为智能体开启新邮件自动处理后，未被邮件访问控制拦截的新邮件会在这里形成事件；处理完成后还会
显示最终摘要和工具执行轨迹。首次启用只建立当前 UID 基线，不补处理历史邮件。完整流程见
[邮箱管理与自动化](./mailbox#自动处理新邮件)。

**邮件管控：**
启用邮箱访问控制后，收件箱会显示 **邮件管控** 入口和待处理数量。你可以按智能体查看未知
发件人，执行通过、拉黑或忽略，并维护精确邮箱地址或 `*@example.com` 域名级白名单与
黑名单。通过待处理发件人后，系统会逐封补处理该记录中保存的所有邮件；失败任务会进入可跨
重启重试的队列。
...
**browser** 工具卡片上还有一个额外按钮，用于切换 **新版(Beta)** / **旧版(兼容)** 两种浏览器实现。它写入全局配置、对所有智能体生效，且需要重启服务才会生效，详见 [浏览器](./browser)。
...
QwenPaw 原生智能体还可以在这里配置 **邮箱管理**：连接已有个人邮箱，或为智能体准备一个
待注册的专用邮箱；同时可选择关闭自动处理或让每封新邮件唤醒智能体。自动处理开启时还可
启用邮件访问控制。第三方智能体后端不支持邮箱配置。配置步骤和支持的服务商见
[邮箱管理与自动化](./mailbox)。
...
- 广播技能到具体的智能体
- 更新内置技能到当前安装包版本
- 在技能详情中独立配置内置技能 **自动更新**（内置技能包 → 技能池）和
  **自动同步**（技能池 → 智能体工作区）
...
配置工具到达转入后台截止时间时的默认行为，可选：

- **保持前台执行**（产品默认）— 转入后台倒计时结束后不自动转入后台，工具继续在对话前台运行，直到自行结束或触达执行超时。
- **自动转入后台** — 倒计时结束后由后端转入后台，智能体可继续处理其他工作。
...
| 工具后台策略 | 设置 → 工具后台策略 | 默认策略：保持前台执行 / 自动转入后台 |
...
- [邮箱管理与自动化](./mailbox) —— 连接邮箱、自动分流和访问控制
