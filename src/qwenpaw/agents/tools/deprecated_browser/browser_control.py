...
"""Deprecated browser automation tool using Playwright.
...
_HEADLESS_VERIFICATION_WARNING = (
    ...
    "the current browser, then call browser with action='start' and "
...
def _should_use_managed_cdp(private_mode: bool, cdp_port: int) -> bool:
    """Return whether this launch explicitly opts in to managed CDP."""
    if private_mode:
        return False
    return cdp_port > 0
...
async def _start_managed_cdp_browser(  # pylint: disable=too-many-statements
    ...
    if await asyncio.to_thread(_probe_local_port_in_use, chosen_cdp_port):
        raise RuntimeError(
            f"Port {chosen_cdp_port} is already in use; refusing to attach "
...
async def _action_start(
    ...
    if cdp_port and private_mode:
        return _tool_response(
            json.dumps(
                {
                    "ok": False,
                    "error": "private_mode=true conflicts with cdp_port; drop one of them.",
                },
                ensure_ascii=False,
                indent=2,
            ),
    ...
    # A new start supersedes a session that is no longer live. Clear its
    # persisted launch configuration before any fallible work so a rejected
    # or failed request cannot make _ensure_browser replay a stale CDP port.
    state["_expose_cdp"] = False
    state["_cdp_port"] = 0
    state["_browser_args"] = ""
    state["_executable_path"] = ""
            ...
            json.dumps(
                {
                    "ok": False,
                    "error": (
                        f"Port {cdp_port} is already in use. Choose a "
                        "different cdp_port or stop the existing process first."
                    ),
                },
                ensure_ascii=False,
                indent=2,
    ...
    try:
        ...
        _start_idle_watchdog(state)
        await _configure_download_behavior(state)
        # Store only the successfully applied launch configuration.
        state["_browser_args"] = browser_args
        state["_executable_path"] = executable_path
        state["_expose_cdp"] = _should_use_managed_cdp(
            private_mode,
            cdp_port,
        )
...
@tool_descriptor(
    ...
    ui_icon="🌐",
    bound_skills=("browser_cdp", "browser_visible"),
    bound_skills_root=str(Path(__file__).resolve().parent / "skills"),
)
async def browser(  # pylint: disable=R0911,R0912
    action: str,
    url: str = "",
    page_id: str = "default",
    selector: str = "",
    ...
    Args:
        ...
        cdp_port (int):
            When > 0 with action=start, launch with a local CDP port so
            another local tool can attach. When 0, no debugging port opens.
        private_mode (bool):
            Retained for compatibility. The default is already direct
            Playwright management with no debugging port; combining it with
            cdp_port is rejected.
        browser_args (str):
            ...
            Remote-debugging switches are rejected; use cdp_port explicitly.
        executable_path (str):
